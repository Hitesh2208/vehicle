...
var _this = $(this)
		...
		$.ajax({
			...
			success:function(resp){
				...
				if ($resp == "Two users with the same username are not allowed") {
    echo '<script>alert("Username already exists");</script>';
} elseif ($resp == "Two users with the same name are not allowed") {
    echo '<script>alert("Name already exists");</script>';
} else {
    // Handle other responses or update UI
    ...
    $("html, body").animate({ scrollTop: 0 }, "fast");
				...
				// }else{
				// 	<?php
				// 	if('#msg'=='Two users with the same name are not allowed'){
				// 		echo '<script>
				// 		alert("Name already exists");
				// 		</script>';
				// 	}
				// 	if('#msg'=='Two users with the same username are not allowed'){
				// 		echo '<script>
